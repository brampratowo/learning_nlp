...
class LanguageTool:
    ...
    @classmethod
    def getSimilarityNGrams(self, textAnswering, textComparing):
        list_grams_aswering = HelperFunction.gramsText(textAnswering, 3)
        list_grams_comparing = HelperFunction.gramsText(textComparing, 3)

        sw = stopwords.words("indonesian")

        grams_answering_set = set()
        grams_comparing_set = set()
        # for grams in list_grams_aswering:
        #     for gram in grams:
                # print(SimilarityHelper.stemmingSentence(gram))
                # grams_answering_set.add()
    ...
    def getValueSimilarity(
        ...
        # tokenization
        answering_token = word_tokenize(textAnswering.lower())
        ...
        answering_set = {
            SimilarityHelper.stemmingSentence(w) for w in answering_token if not w in sw
        }
        comparing_set = {
